...
import { CmdSupplierProvider } from "./CmdSupplierContext"
import { CmdSupplierSearch } from "./CmdSupplierSearch"
...
import type { SupplierOrder, SupplierOrderRequest, OrderStatus } from "@/types/supplier/supplierOrder"
...
interface AdminCmdSupplierContentProps {
  readonly currentUser: { roleName: string } | null
  readonly ordersData: SupplierOrder[]
  readonly displayData: SupplierOrder[]
  readonly stats: {
  ...
  readonly selectedOrders: SupplierOrder[]
  readonly isCreateModalOpen: boolean
  readonly editingOrder: SupplierOrder | null
  readonly setIsCreateModalOpen: (open: boolean) => void
  readonly setFilteredOrders: (orders: SupplierOrder[]) => void
  readonly setHasFilter: (hasFilter: boolean) => void
  readonly handleEditOrder: (order: SupplierOrder) => void
  readonly handleRowSelectionChange: (selection: unknown) => void
  readonly clearSelection: () => void
  readonly setCurrentPage: (page: number) => void
  readonly setEditingOrder: (order: SupplierOrder | null) => void
  ...
  readonly handleFormSubmit: (data: SupplierOrderRequest) => Promise<void>
...
export function AdminCmdSupplierContent({
  currentUser,
  ordersData,
  displayData,
  stats,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Commandes Fournisseurs</h1>
          <p className="text-sm sm:text-base text-muted-foreground">
            Gérez les commandes de vos fournisseurs
          </p>
        </div>
        ...
        <CardContent className="space-y-4 px-4 sm:px-6">
          ...
          <div className="overflow-x-auto">
            {displayData.length === 0 ? (
              ...
              <CmdSupplierProvider onEditOrder={handleEditOrder}>
                <DataTable 
                  columns={columns} 
                  data={displayData}
