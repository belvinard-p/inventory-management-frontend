...
interface CmdClientSearchProps {
  readonly data: ClientOrderResponse[]
  readonly onFilteredData: (filtered: ClientOrderResponse[], hasFilter?: boolean) => void
  readonly placeholder?: string
}

export function CmdClientSearch({ 
  data, 
  onFilteredData, 
  placeholder = "Rechercher commande..." 
}: CmdClientSearchProps) {
  ...
  const handleFilterChange = (value: string) => {
    ...
    const filtered = data.filter(order => {
      switch (value) {
        ...
        case "with-lines":
          return order.orderClientLineList && order.orderClientLineList.length > 0
        case "without-lines":
          return !order.orderClientLineList || order.orderClientLineList.length === 0
        default:
          return true
