...
// Définir les transitions autorisées selon la logique backend
// COMPLETED peut maintenant seulement revenir à CONFIRMED
...
export function CmdClientStatusDialog({
    order,
    open,
    onOpenChange,
    onStatusChange,
    isLoading = false,
}: CmdClientStatusDialogProps) {
    ...
    // Debug: Afficher les transitions autorisées
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-md">
                ...
                <div className="space-y-6 py-4">
                    ...
                    {/* Information sur les transitions */}
                    <div className="flex items-start gap-2 p-3 rounded-lg bg-blue-50 border border-blue-200">
                        <Info className="h-5 w-5 text-blue-600 mt-0.5 flex-shrink-0" />
                        <div className="text-sm text-blue-900">
                            <p className="font-medium">Transitions disponibles</p>
                            <p className="text-blue-700 mt-1">
                                Seules les transitions autorisées par les règles métier sont disponibles. Les modifications sont limitées à 7 jours.
                            </p>
                        </div>
                    </div>

                    {/* Avertissement si le statut sélectionné n'est pas autorisé */}
                    {hasChanges && !isSelectedStatusAllowed && (
                        <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200">
                            <AlertTriangle className="h-5 w-5 text-amber-600 mt-0.5 flex-shrink-0" />
                            <div className="text-sm text-amber-900">
                                <p className="font-medium">Transition non autorisée</p>
                                <p className="text-amber-700 mt-1">
                                    Le changement de statut sélectionné n&apos;est pas autorisé selon les règles métier.
                                </p>
                            </div>
                        </div>
                    )}
                ...
                <div className="flex justify-end gap-3 pt-4 border-t">
                    ...
                    <Button
                        ...
                        disabled={!hasChanges || !isSelectedStatusAllowed || isUpdating}
                    >
